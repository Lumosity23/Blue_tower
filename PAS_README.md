import pygame 
 # cache simple pour ne pas recharger les images du disque à chaque fois _tture_cache = {}  dget_custom_sprite(path, size, shape='square'):   """   Charge une image, la redimensionne et applique une forme.   shape: 'square' ou 'circle'
"""
...
try:
    ...
    return pygame.Surface(size) # Retourne une surface vide ou rose par défaut
...
return image
  ...
  Utilisation dans tes classes :

   1 from utils import get_custom_sprite
   2 
   3 class Enemy(pygame.sprite.Sprite):
   4     def __init__(self):
   5         super().__init__()
   6         # Je veux un rond de 50x50
   7         self.image = get_custom_sprite('assets/enemie.png', (50, 50), shape='circle')
   8         self.rect = self.image.get_rect()# Blue_tower
