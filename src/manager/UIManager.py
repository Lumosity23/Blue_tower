...
from ui.panel.CharacterPanel import CharacterPanel
from ui.panel.DemoUIPanel import DemoUIPanel
...
class UIManager:
    def __init__(self, game: "App"):
        ...
        UIElement.get_save(self.game.save_manager)

        # Init de notre bloc logic pour le UI
        self.root = UIElement(
            0, 0, self.st.SCREEN_WIDTH, self.st.SCREEN_HEIGHT, uid="ROOT"
        )
        ...
        self.character = CharacterPanel(game)
        self.demo_ui = DemoUIPanel(game)
        ...
        self.game.eventManager.subscribe("SHOW_CHARACTER", self.on_character)
        self.game.eventManager.subscribe("SHOW_DEMO_UI", self.on_demo_ui)
    ...
    def on_demo_ui(self) -> None:
        for child in self.root.children:
            child.visible = False
        self.demo_ui.visible = True
    ...
    def save_tree(self, parent: "UIElement") -> None:
        ...
        with open(rp(self.tree_path), "w") as f:
            json.dump(
                data_to_save, f, indent=4
            )  # indent=4 pour que ce soit joli à lire
