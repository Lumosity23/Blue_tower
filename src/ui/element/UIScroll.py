...
class UIScroll(UIElement):
    ...
    """
    def __init__(self, x: int, y: int, w: int, h: int, color=(30, 30, 30), uid: str = None):
    ...
    def update_content_size(self) -> None:
        ...
        self.max_scroll_y = max(0, max_y - self.rect.h)
        self.max_scroll_x = max(0, max_x - self.rect.w)

        # On contraint la cible si les limites ont changé
        self.target_scroll_offset.y = max(0, min(self.target_scroll_offset.y, self.max_scroll_y))
        self.target_scroll_offset.x = max(0, min(self.target_scroll_offset.x, self.max_scroll_x))
    ...
    def update(self, dt: float) -> None:
        ...
        if self.scroll_offset.distance_to(self.target_scroll_offset) > 0.5:
            ...
            self.scroll_offset += (self.target_scroll_offset - self.scroll_offset) * self.lerp_speed * dt
        else:
