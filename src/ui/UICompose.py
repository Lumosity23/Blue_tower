...
class UICompose(UIElement):
    '''
        ...
        '''
    def __init__(self, x: int, y: int, w: int, h: int, label_text: str, sprite: pygame.Surface, action_button, text_button: str | None = None, uid: str | None = None):
        super().__init__(x, y, w, h, uid)

        child_uid = f"{uid}_" if uid else ""

        ## LABEL
        self.label = UIText(0, 10, label_text, size_text=25, uid=f"{child_uid}label")
        # Centrage horizontal
        ...
        ## BOUTON ACTION
        self.button = UIButton(0, 0, 0, 0, text_button, action_button, (25, 100, 155), size_text=20, uid=f"{child_uid}btn")
    ...
    def set_info_button(self, text: str) -> None:
        ...
        if not self.info_button:
            ...
            self.info_button = UIButton(
                ...
                "i", self.show_description, (0, 150, 255), size_text=15, uid=f"{self.uid}_info"
            )
